//
//  AddressPressListCollectViewController.swift
//  V608
//
...
class AddressPressListCollectViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func updateTableView() {
        DispatchQueue.main.async {[weak self] in
            self?.regionItems = RequestManager.shared.regionItems.filter( {$0.regioncCollected == true})
    ...
    lazy var regionItems: [RegionItem] = {
        return RequestManager.shared.regionItems.filter( {$0.regioncCollected == true})
    }()
    
}

extension AddressPressListCollectViewController {
